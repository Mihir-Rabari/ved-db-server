...
impl Command {
    pub fn new(op: OpCode, seq: u32, key: Vec<u8>, value: Vec<u8>) -> Self {
        let header = CmdHeader::new(op, seq)
            .with_key_val(key.len() as u32, value.len() as u32);
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        let mut bytes = Vec::with_capacity(
            CmdHeader::SIZE + self.key.len() + self.value.len()
        );
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, &'static str> {
        ...
        let header = unsafe {
            std::ptr::read_unaligned(bytes.as_ptr() as *const CmdHeader)
        };
...
impl Response {
    pub fn new(status: Status, seq: u32, payload: Vec<u8>) -> Self {
        let header = RespHeader::new(status, seq)
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, &'static str> {
        ...
        let header = unsafe {
            std::ptr::read_unaligned(bytes.as_ptr() as *const RespHeader)
        };
        ...
        let payload = bytes[RespHeader::SIZE..RespHeader::SIZE + header.payload_len as usize].to_vec();
