...
mod tests {
    ...
    fn test_subscriber_list() {
        ...
        let list = unsafe { SubscriberList::init(memory.as_mut_ptr(), 10) };
    ...
    fn test_subscriber_list_capacity() {
        ...
        let list = unsafe { SubscriberList::init(memory.as_mut_ptr(), 2) };
        ...
        assert_eq!(
            list_ref.add_subscriber(3),
            Err(PubSubError::TooManySubscribers)
        );
