...
use super::{PubSubError, Topic, TopicDesc, PubSubConfig};
...
impl TopicRegistry {
    ...
    pub fn create_topic(
        ...
        // Find empty slot
        let entry_index = self.find_empty_entry()
            .ok_or(PubSubError::TooManyTopics)?;
    ...
    pub fn delete_topic(&self, name: &str, arena: *mut Arena) -> Result<(), PubSubError> {
        ...
        let entry_index = self.find_topic_entry(name)
...
mod tests {
    ...
    fn test_topic_registry() {
        ...
        let registry = unsafe {
            TopicRegistry::init(registry_memory.as_mut_ptr(), 10)
        };
