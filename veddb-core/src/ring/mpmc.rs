...
impl MpmcRing {
    ...
    pub fn try_pop(&self) -> Option<Slot> {
        ...
        unsafe {
            ...
            slot.sequence
                .store(expected_seq + self.capacity - 1, Ordering::Release);
...
impl MpmcRingBuffer {
    ...
    pub fn new(capacity: u64) -> Self {
        ...
        // We need 64-byte alignment for cache-line aligned atomics
        let layout = std::alloc::Layout::from_size_align(size, 64).unwrap();
        let ptr = unsafe { std::alloc::alloc_zeroed(layout) };
        if ptr.is_null() {
            panic!("Failed to allocate memory for MPMC ring");
        }
        let memory = unsafe { Vec::from_raw_parts(ptr, size, size) };

        let ring = unsafe { MpmcRing::init(memory.as_ptr() as *mut u8, capacity) };
...
impl Drop for MpmcRingBuffer {
    fn drop(&mut self) {
        // We need to properly deallocate the aligned memory
        let size = self.memory.len();
        let layout = std::alloc::Layout::from_size_align(size, 64).unwrap();
        let ptr = self.memory.as_mut_ptr();
        
        // Prevent Vec's drop from trying to deallocate
        std::mem::forget(std::mem::replace(&mut self.memory, Vec::new()));
        ...
        for producer_id in 0..num_producers {
            ...
            let handle = thread::spawn(move || {
                for i in 0..items_per_producer {
                    ...
                    let slot = Slot::arena_offset(value as u32, value);
                    ring.ring().push(slot);
                }
            });
        ...
        let producer = thread::spawn({
            ...
            move || {
                for i in 0..total_items {
                    let slot = Slot::arena_offset(i as u32, i);
                    ring.ring().push(slot);
                }
            }
        });
            ...
            let handle = thread::spawn(move || {
                ...
                loop {
                    if let Some(slot) = ring.ring().try_pop() {
                        ...
                        if !received.is_empty() && ring.ring().is_empty() {
                            break;
                        }
                    }
                }
