...
impl HashTable {
        ...
        unsafe {
            ...
            if is_match {
                ...
                let new_offset = if new_size > old_size {
                    ...
                    if off == 0 { return Err(KvError::OutOfMemory); }
                    off
                } else {
                    old_offset
                };
    ...
    pub fn remove(&mut self, key: &[u8]) -> Option<usize> {
        ...
        if is_match {
            unsafe {
                ...
                {
                    let entry = self.entry_at_mut(index);
                    if entry.is_tombstone() { return None; }
    ...
    pub fn cas(&mut self, key: &[u8], expected_version: u64, new_value: &[u8]) -> Result<u64, KvError> {
        let hash = self.hash_key(key);
        let (index, is_match) = self.find_entry(key, hash);
        ...
        unsafe {
            ...
            // Copy key and new value
            {
                ...
                std::ptr::copy_nonoverlapping(new_value.as_ptr(), data_ptr.add(key_len), new_value.len());
            }
...
mod tests {
    ...
    fn test_hash_table_basic_operations() {
        ...
        let table = unsafe {
            HashTable::init(table_memory.as_mut_ptr(), 16, arena)
        };
    ...
    fn test_hash_table_collisions() {
        ...
        let table = unsafe {
            HashTable::init(table_memory.as_mut_ptr(), 4, arena)
        };
        ...
        for i in 0..8 {
            ...
            assert_eq!(table_ref.get(key.as_bytes()).unwrap(), expected_value.as_bytes());
        }
    }
}
