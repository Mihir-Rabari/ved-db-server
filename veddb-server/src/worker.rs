    ...
    async fn run_worker(
        ...
        loop {
            ...
            if stats.active_sessions > 0 {
                ...
                had_work = Self::process_session_commands(&veddb, id).await?;
            }
            ...
            if last_report.elapsed() > Duration::from_secs(60) {
                info!("Worker {} processed {} commands in last minute", id, processed_count);
                processed_count = 0;
                last_report = std::time::Instant::now();
            }
        }
        
        info!("Worker {} shutdown complete", id);
        Ok(())
    }
    
    /// Process commands from sessions assigned to this worker
    async fn process_session_commands(veddb: &VedDb, worker_id: usize) -> Result<bool> {
        let mut had_work = false;
        
        // Get all active sessions and process commands
        let session_ids = veddb.get_active_sessions();
        
        for session_id in session_ids {
            // Only process sessions assigned to this worker (simple round-robin)
            if (session_id as usize) % num_cpus::get() != worker_id {
                continue;
            }
            
            // Drain all available commands for this session to amortize costs
            loop {
                match veddb.try_get_command(session_id)? {
                    Some(command) => {
                        debug!("Worker {} processing command from session {}", worker_id, session_id);
                        let response = veddb.process_command(command);
                        if let Err(e) = veddb.send_response(session_id, response) {
                            warn!("Worker {} failed to send response to session {}: {}", worker_id, session_id, e);
                        }
                        had_work = true;
                    }
                    None => break,
