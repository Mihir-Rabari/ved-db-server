...
async fn main() -> Result<()> {
    ...
    let env_filter = EnvFilter::new(format!("veddb_server={},veddb_core={}", log_level, log_level));
    tracing_subscriber::fmt()
        .with_env_filter(env_filter)
        .init();
    
    info!("Starting VedDB Server");
    info!("Memory: {}MB, Workers: {}, Port: {}", args.memory_mb, args.workers, args.port);
    ...
    info!("VedDB initialized - Memory: {:.1}MB used / {:.1}MB total", 
    ...
    let grpc_handle = tokio::spawn(async move {
        grpc_server.serve(args.port).await
    });
    ...
    let stats_handle = tokio::spawn(async move {
        ...
        loop {
            ...
            info!("Stats - Ops: {}, Keys: {}, Sessions: {}, Topics: {}, Memory: {:.1}MB", 
    ...
    info!("Final stats - Total operations: {}, Uptime: {}s", 
